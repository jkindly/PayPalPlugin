...
{
    /**
     * PayPal only accepts tracking for orders in one of these statuses; guarding here means we do
     * not waste a call that PayPal would reject.
     */
    ...
    public function process(ShipmentInterface $shipment): void
    {
        ...
        if ($tracking === null) {
            // No carrier was ever selected for this shipment - nothing to send to PayPal.
            return;
        }
        ...
        if ($payment === null) {
            // Order was not paid with PayPal - leave the record untouched.
            return;
        }
        ...
        try {
            ...
            // Failure isolation: record the failure with enough detail to retry, keep the shipment shipped.
    ...
    private function resolveCaptureId(array $paymentDetails, array $orderDetails): ?string
    {
        ...
        return $captureId !== null ? (string) $captureId : null;
    }
