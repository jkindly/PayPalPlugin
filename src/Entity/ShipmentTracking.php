...
use Sylius\PayPalPlugin\Repository\ShipmentTrackingRepository;

/**
 * Plugin-owned tracking record keyed by shipment. It holds the carrier, the PayPal tracker
 * identifier and the sync state, so that no field has to be added to the core Shipment entity
 * (which would force every shop to override it on upgrade). The tracking number itself is not
 * duplicated here - it is read from the core Shipment::getTracking().
 */
