...
final class ShipmentTrackingProcessorTest extends TestCase
{
    ...
    public function it_includes_the_free_text_carrier_name_for_the_other_carrier(): void
    {
        ...
        $this->addTrackingApi
            ...
            ->with('TOKEN', 'ORDER123', self::callback(
                fn (array $body): bool => ($body['carrier'] ?? null) === 'OTHER' && ($body['carrier_name_other'] ?? null) === 'Local Courier',
            ))
            ->willReturn([]);
    ...
    #[Test]
    public function it_records_the_failure_and_does_not_rethrow_when_paypal_errors(): void
    {
        $shipment = $this->shipment('TRACK1');
        $tracking = $this->trackingFor($shipment, 'FEDEX');
    ...
    private function shipment(string $trackingNumber): ShipmentInterface&MockObject
    {
        $shipment = $this->createMock(ShipmentInterface::class);
