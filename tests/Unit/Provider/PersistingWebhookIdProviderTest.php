...
final class PersistingWebhookIdProviderTest extends TestCase
{
    ...
    public function test_returns_the_webhook_id_stored_in_the_gateway_config_without_querying(): void
    {
    ...
    public function test_resolves_and_stores_the_webhook_id_when_it_is_missing_from_the_config(): void
    {
    ...
    public function test_returns_null_and_stores_nothing_when_the_lookup_finds_no_webhook(): void
    {
        $gatewayConfig = $this->gatewayConfig([]);
        $paymentMethod = $this->paymentMethod($gatewayConfig);
    ...
    public function test_refreshes_from_a_live_lookup_and_stores_it_when_cooldown_allows(): void
    {
        $gatewayConfig = $this->gatewayConfig(['webhook_id' => 'WH-STALE']);
        $paymentMethod = $this->paymentMethod($gatewayConfig);

        $this->mockCooldown(false);
    ...
    public function test_does_not_refresh_while_within_the_cooldown_window(): void
    {
        $paymentMethod = $this->paymentMethod($this->gatewayConfig(['webhook_id' => 'WH-STALE']));

        $this->mockCooldown(true);
